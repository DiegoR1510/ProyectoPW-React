...
const TopRated: React.FC = () => {
  ...
  const allGenres = Array.from(new Set(topRated.flatMap(g => Array.isArray(g.genre) ? g.genre : []).map(g => g && g.trim()).filter(Boolean)));
  const allPlatforms = Array.from(new Set(topRated.flatMap(g => Array.isArray(g.platform) ? g.platform : []).map(p => p && p.trim()).filter(Boolean)));

  const filteredGames = topRated.filter(game => {
